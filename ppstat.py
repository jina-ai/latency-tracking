...
stats_file = sys.argv[2] or '/var/output/stats.json'

with open(stats_file) as fp:
    d = json.load(fp)

x = PrettyTable()
x.field_names = ['Version', 'Index QPS',
                 'Query QPS', 'DAM Extend QPS', 'Avg Flow Time (s)', 'Import Time (s)']
...
x.add_row([f'current', index_qps, query_qps, dam_extend_qps, avg_flow_time, import_time])
for dd in d[:-1][::-1]:
    x.add_row([f'[`{dd["version"]}`](https://github.com/jina-ai/jina/tree/v{dd["version"]})',
              int(dd['index_qps']), int(dd['query_qps']), int(dd['dam_extend_qps']), round(dd['avg_flow_time'], 4), round(dd['import_time'], 4)])
